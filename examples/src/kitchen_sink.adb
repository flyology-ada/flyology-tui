...
procedure Kitchen_Sink is
   ...
   function Selector_Appearance (Item : Model) return Selectors.Appearance is
     (Normal   => Active_Skin (Item).Control,
      Selected => Active_Skin (Item).Control_Selected,
      Focused  => Active_Skin (Item).Control_Focused,
      Disabled => Current_Palette (Item).Disabled);

   function Dropdown_Appearance (Item : Model) return Dropdowns.Appearance is
     (Normal      => Active_Skin (Item).Control,
      Selected    => Active_Skin (Item).Control_Selected,
      Highlighted => Current_Palette (Item).Button_Focused,
      Focused     => Active_Skin (Item).Control_Focused,
      Disabled    => Current_Palette (Item).Disabled);

   function Menu_Appearance (Item : Model) return Menus.Appearance is
     (Bar         => Active_Skin (Item).Menu_Bar,
      Menu        => Active_Skin (Item).Dialog,
      Focused     => Current_Palette (Item).Button_Focused,
      Highlighted => Active_Skin (Item).Control_Selected,
      Disabled    => Current_Palette (Item).Disabled,
      Separator   => Current_Palette (Item).Border,
      Shortcut    => Current_Palette (Item).Muted,
      Marker      => Current_Palette (Item).Success);
   ...
   function Visible_Dropdown_Bounds
     ...
     (Intersect
        ((X      => Geometry.Dropdown_Origin.X,
         ...
         Panel_Content (Item, Geometry.Fourth)));
   ...
   begin
      if Geometry.Right_Full.Width > 0 then
         Text_Region := Panel_Content (Item, Geometry.Left_Full);
         Syntax_Region := Panel_Content (Item, Geometry.Right_Full);
      else
         Text_Region := Panel_Content (Item, Geometry.Top_Full);
         Syntax_Region := Panel_Content (Item, Geometry.Bottom_Full);
      end if;
   end Editor_Regions;
   ...
   begin
      ...
      if Item.Skin = Flyology_TUI.Skins.Turbo_Vision then
         --  Late Turbo Vision applications put the menu at row zero and the
         --  status line at the bottom; they do not reserve a website-like
         --  application banner above the menu.
         Result.Header := (0, 0, Result.Width, 0);
         Result.Tabs :=
           (X => 0, Y => 0, Width => Result.Width,
            Height => (if Result.Height > 0 then 1 else 0));
         Result.Help :=
           (X => 0,
            Y => (if Result.Height > 1 then Integer (Result.Height - 1)
                  else Integer (Result.Height)),
            Width => Result.Width,
            Height => (if Result.Height > 1 then 1 else 0));
         Content_Height :=
           (if Result.Height > 2 then Result.Height - 2 else 0);
         Result.Content :=
         ...
         Content_Height :=
           (if Result.Height > 3 then Result.Height - 3 else 0);
         Result.Content :=
      ...
      Result.Text_Content := Panel_Content (Item, Result.First);
      Result.Text_Content.Height :=
        Natural'Min (1, Result.Text_Content.Height);
      Result.List_Content := Panel_Content (Item, Result.Third);
      declare
         Viewport_Frame : constant Flyology_TUI.Geometry.Rectangle :=
           Panel_Content (Item, Result.Second);
      ...
      Result.Form_Content := Panel_Content (Item, Result.Fourth);
      Result.Button_Origin := Origin (Panel_Content (Item, Result.First));
      Result.Check_Origin :=
        (X => Result.Button_Origin.X,
         Y => Result.Button_Origin.Y
           + Integer
               (Flyology_TUI.Surfaces.Height
                  (Item.Button.Render
                     (Active_Skin (Item), Item.Focus = Button_Field)))
           + 1);
      Result.Radio_Origin := Origin (Panel_Content (Item, Result.Second));
      Result.Selector_Origin := Origin (Panel_Content (Item, Result.Third));
      Result.Dropdown_Origin := Origin (Panel_Content (Item, Result.Fourth));
      Result.Telemetry_Origin := Origin (Panel_Content (Item, Result.First));
      Result.Breadcrumb_Origin := Origin (Panel_Content (Item, Result.First));
      Result.Table_Origin := Origin (Panel_Content (Item, Result.Third));
      Result.Tree_Origin := Origin (Panel_Content (Item, Result.Second));
      Result.Accordion_Origin := Origin (Panel_Content (Item, Result.Fourth));
      if Result.Right_Full.Width > 0 then
         Result.Text_Area_Origin :=
           Origin (Panel_Content (Item, Result.Left_Full));
         Result.Syntax_Origin :=
           Origin (Panel_Content (Item, Result.Right_Full));
      else
         Result.Text_Area_Origin :=
           Origin (Panel_Content (Item, Result.Top_Full));
         Result.Syntax_Origin :=
           Origin (Panel_Content (Item, Result.Bottom_Full));
      end if;
   ...
   begin
      Editor_Regions (Item, Geometry, Editor_Region, Syntax_Region);
      ...
      case Item.Capture is
            ...
            declare
               Presentation : constant Accordions.Presentation :=
                 Accordion_Presentation
                   (Item, Panel_Content (Item, Geometry.Fourth).Width);
            begin
               ...
               Presentation : constant Menus.Presentation :=
                 Item.Menu.Present
                   (Geometry.Menu_Frame.Width,
                    ...
                    0, 0, Menu_Appearance (Item), Item.Focus = Menu_Field);
         ...
         when First_Window_Capture =>
            ...
            Result := Item.Window_A_Model.Handle
              (Page_Event, Geometry.Window_Workspace, Active_Skin (Item));
   ...
   procedure Handle_Editors_Mouse
     (Item  : in out Model;
      ...
      Result : Update_Result;
   begin
      Editor_Regions (Item, Geometry, Text_Region, Syntax_Region);
   ...
   function Panel
     (Skin       : Flyology_TUI.Skins.Skin;
      ...
      return Flyology_TUI.Surfaces.Surface
   is
      function Dialog_Frame return Flyology_TUI.Styles.Style is
         Result : Flyology_TUI.Styles.Style := Skin.Palette.Border;
      begin
         Result.Background := Skin.Dialog.Background;
         return Result;
      end Dialog_Frame;
      ...
      Body_Surface : constant Flyology_TUI.Surfaces.Surface :=
        (if Skin.Panel.Title = Flyology_TUI.Skins.Centered_Title
         then Content
         else Flyology_TUI.Layouts.Join_Vertically
           (Heading, Content, Gap => 1));
   ...
   begin
      if Skin.Panel.Title = Flyology_TUI.Skins.Centered_Title
        and then Result.Width > 2 and then Result.Height > 0
      then
         declare
            Title_Text : constant Wide_Wide_String := " " & Name & " ";
            Available : constant Natural := Result.Width - 2;
            Title_Width : constant Natural := Natural'Min
              (Available, Flyology_TUI.Glyphs.Width_Of (Title_Text));
            Title_X : constant Natural := 1 + (Available - Title_Width) / 2;
            Title_Surface : Flyology_TUI.Surfaces.Surface :=
              Flyology_TUI.Surfaces.Create
                (Title_Width, 1, Skin.Dialog);
         begin
            Title_Surface.Write (0, 0, Title_Text, Skin.Palette.Title);
            Result.Overlay_Clipped
              (Title_Surface, Integer (Title_X), 0);
         end;
      end if;
      return Result;
   end Panel;
   ...
   begin
      ...
      Result.Overlay_Clipped
        (Item.Vertical_Scroll.Render (Active_Skin (Item)),
         Geometry.Vertical_Scroll_Region.X - Frame.X,
         Geometry.Vertical_Scroll_Region.Y - Frame.Y);
      Result.Overlay_Clipped
        (Item.Horizontal_Scroll.Render (Active_Skin (Item)),
   ...
   function Basics_View
     (Item : Model; Geometry : Layout_Snapshot)
      ...
      List_View : constant Flyology_TUI.Surfaces.Surface :=
        Panel
          (Active_Skin (Item), "Generic list",
           Item.Choices.Render
             (Active_Skin (Item).Control,
              Active_Skin (Item).Control_Selected),
      ...
      Action_Content : constant Flyology_TUI.Surfaces.Surface :=
        Flyology_TUI.Layouts.Join_Vertically
          (Item.Button.Render
             (Active_Skin (Item), Item.Focus = Button_Field),
           Item.Check.Render
             (Active_Skin (Item), Item.Focus = Check_Field),
           Gap => 1);
      ...
      Radio_View : constant Flyology_TUI.Surfaces.Surface :=
        Panel
          (Active_Skin (Item), "Radio group",
           Item.Radios.Render
             (Active_Skin (Item), Item.Focus = Radio_Field),
        ...
        Panel
          (Active_Skin (Item), "Bounded series",
           Gradient_Marks
             (Sparklines.Render
                (Item.Samples,
                 Panel_Content (Item, Geometry.Second).Width,
      ...
      Status : constant Flyology_TUI.Surfaces.Surface :=
        Indicators.Status_Line
          ([Indicators.Make_Segment
           ...
           Panel_Content (Item, Geometry.Third).Width,
           Theme);
              ...
              Flyology_TUI.Layouts.Join_Vertically
                (Gradient_Marks
                   (Item.Work.Render_Segments
                      (Panel_Content (Item, Geometry.Third).Width, Theme),
                    Item.Deep_Gradient),
                 Status),
              Gap => 1));
   ...
   function Present (Item : Model) return Flyology_TUI.Views.View is
      ...
      Header : constant Flyology_TUI.Surfaces.Surface :=
        Header_Raw.Inherit_Colors (Active_Skin (Item).Menu_Bar);
      ...
      Help_Raw : constant Flyology_TUI.Surfaces.Surface :=
        Flyology_TUI.Components.Help.Render
          ([(Key => U
           ...
           Key_Appearance => Current_Palette (Item).Interaction,
           Detail_Appearance => Active_Skin (Item).Status_Line);
      Help : constant Flyology_TUI.Surfaces.Surface :=
        Help_Raw.Inherit_Colors (Active_Skin (Item).Status_Line);
      Canvas : Flyology_TUI.Surfaces.Surface :=
        Flyology_TUI.Surfaces.Create
   ...
   begin
      ...
      Assert
        (Geometry.Header.Height = 0
         and then Geometry.Tabs.Y = 0
         and then Geometry.Tabs.Height = 1
         and then Geometry.Content.Y = 1,
         "Turbo layout did not place the menu on the first row");
      declare
      ...
      begin
         Assert
           (Text.To_Wide_Wide_String
            ...
            and then Pages.Frame (Skin_Tabs).Element
              (1, 0).Appearance.Background =
                Flyology_TUI.Colors.True_Color (0, 0, 168)
            and then Pages.Frame (Skin_Tabs).Element
              (1, 0).Appearance.Reverse_Video,
      ...
      Assert
        (Text.To_Wide_Wide_String
           (Frame.Frame.Element
              (Natural (Geometry.Check_Origin.X),
               Natural (Geometry.Check_Origin.Y)).Glyph) = "[",
         "Turbo check hit origin did not match its rendered row");
      Handle_Mouse
        (Item,
         (Kind  => Flyology_TUI.Events.Mouse_Input,
          Mouse => Pointer
            (Natural (Geometry.Check_Origin.X),
             Natural (Geometry.Check_Origin.Y))));
      Handle_Mouse
        (Item,
         (Kind  => Flyology_TUI.Events.Mouse_Input,
          Mouse => Pointer
      ...
      Assert
        (Item.Check.State = Flyology_TUI.Components.Check_Boxes.Checked
         and then Item.Focus = Check_Field
         and then Item.Capture = No_Capture,
         "Turbo check did not respond at its rendered mouse position");

      Activate_Page (Item, Navigation_Page);
      Set_Terminal_Size (Item, 112, 30);
      Geometry := Layout (Item);
      Frame := Present (Item);
      Assert
        (Text.To_Wide_Wide_String
           (Frame.Frame.Element
              (Natural (Geometry.Tree_Origin.X),
               Natural (Geometry.Tree_Origin.Y)).Glyph) =
           [1 => Wide_Wide_Character'Val (16#25BE#)],
         "Turbo tree hit origin did not match its rendered row");
      Handle_Mouse
        (Item,
         (Kind  => Flyology_TUI.Events.Mouse_Input,
          Mouse => Pointer
            (Natural (Geometry.Tree_Origin.X),
             Natural (Geometry.Tree_Origin.Y + 1))));
      Assert
        (Item.Tree.Selected_Id = Source_Node
         and then Item.Focus = Tree_Field,
         "Turbo tree mouse routing did not select the rendered row");
      Handle_Focused_Key
        (Item,
         Terminal_Key (Flyology_TUI.Events.Arrow_Down_Key), Geometry);
      Assert
        (Item.Tree.Selected_Id = Components_Node,
         "Turbo tree keyboard navigation did not advance selection");
